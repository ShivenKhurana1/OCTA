...
class Ball {
  ...
  update() {
    ...
    // ── Border Check: If ball escapes past boundary, return to player ──
    const maxRadius = (CONSTANTS.OCTAGON_SIZE / 2) + 120; // safe threshold beyond normal bounds
    const dx = this.sprite.x - this.game.world.centerX;
    const dy = this.sprite.y - this.game.world.centerY;
  ...
  teleportToPlayer() {
    // 1. Play return sound cue
    this.state.soundManager.play('hit');

    // 2. Replenish ammunition/ball back to the player
    this.state.player.giveBall(this.type);
    this.state.setBallsOut(false);
    this.state.updateCursor();

    // 3. Destroy this escaped ball instance
    this.destroy();
  }
